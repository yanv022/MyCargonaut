...
} from "src/app/components/modals/request-accept-modal/request-accept-modal.component";
...
export class AnfragenService {
  ...
  protected processNewRequest(data:any){
    ...
    this.requestCollection.doc(id).set(
        data
    )
  }
  ...
  protected async requestAccepted(data: any) {
    try{
      ...
      let userCars: any[];
      userCars = await new Promise((resolve) => {this.carsService.getCarsOfUser(uid).subscribe((
          cars) => {
          resolve(cars);
      })})
      if(userCars == undefined || userCars.length == 0){
        this.alertService.newAlert('Bitte fügen Sie zuerst ein Auto hinzu', 'danger');
        return;
      }
