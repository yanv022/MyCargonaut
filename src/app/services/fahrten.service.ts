...
export class FahrtenService {
  ...
  getAllRides() {
    return this.afs
      .collection(this.fahrtenCollection.ref, (ref) =>
  ...
  public async addRide(ride: any, id?: any) {
    ...
    await this.fahrtenCollection.doc(id).set(ride);
  }
  ...
  acceptRide(fahrt: any) {
    ...
    modalRef.result
      .then(async () => {
        ...
        await this.fahrtenCollection.doc(fahrt.id).update({
          accepted: true
        }).then(()=>{
          this.helperService.addRideForPassengerAndDriver(fahrt.creatorId, uid, fahrt.id)
        })
      })
      .catch((error) => {
        console.log(error);
