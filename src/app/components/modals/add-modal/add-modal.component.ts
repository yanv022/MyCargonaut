...
export class AddModalComponent implements OnInit {
              ...
              if (this.cars != undefined) {
                if (this.cars.length === 0) {
                  this.alertService.newAlert(
                    'Sie brauchen mindestens ein Auto',
                    'danger'
                  );
  ...
  async submitData() {
    try {
      ...
      if (
        isNaN(timestamps.timestampAbfahrt.nanoseconds) ||
        isNaN(timestamps.timestampAbfahrt.seconds) ||
        isNaN(timestamps.timestampAnkunft.nanoseconds) ||
        isNaN(timestamps.timestampAnkunft.seconds)
      ) {
        this.alertService.newAlert(
          'Datum und Zeit brauchen korrekte Werte',
          'danger'
        );
        return;
      }
      ...
      if (this.indicator === 'fahrt') {
        if (
          ...
          dataToSubmit = {
            abfahrt: timestamps.timestampAbfahrt,
            wo: this.where,
            ankunft: timestamps.timestampAnkunft,
            wohin: this.to,
            name: this.title,
            creatorId: uid,
  ...
  makeTime() {
    const dateAnkunft = new Date(
      this.dateAnkunft.year,
      this.dateAnkunft.month,
      this.dateAnkunft.day,
      this.timeAnkunft.hour,
      this.timeAnkunft.minute
    );
    const timestampAnkunft = firebase.firestore.Timestamp.fromDate(dateAnkunft);
    const dateAbfahrt = new Date(
      this.dateAbfahrt.year,
      this.dateAbfahrt.month,
      this.dateAbfahrt.day,
      this.timeAbfahrt.hour,
      this.timeAbfahrt.minute
    );
